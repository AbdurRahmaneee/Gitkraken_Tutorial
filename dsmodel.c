...
/* DSblock model generated by Dymola from Modelica model FM3217_2023.Tutorial3.Tests.DCMachineTest
 Dymola Version 2020 (64-bit), 2019-04-10 translated this at Wed Sep 20 23:14:56 2023
...
#define NHash1_ -1824586803
#define NHash2_ -8068151
#define NHash3_ 0
#define NI_    0
#define NRelF_ 0
#define NRel_  0
...
PreNonAliasDef(5)
#if !defined(DYM2CCUR)
 DYMOLA_STATIC const char*modelName="FM3217_2023.Tutorial3.Tests.DCMachineTest";
#endif
...
AssertModelica(1+DYNX(DP_,2)*(DYNX(W_,3)-DYNX(DP_,1)) >= 1E-15,"1+dCMachine.resistor.alpha*(dCMachine.resistor.T-dCMachine.resistor.T_ref) >= 1E-15",
...
DYNX(W_,1) = DYNX(W_,17)-DYNX(W_,0);
DYNX(W_,7) = DYNX(DP_,4)*DYNX(X_,2);
DYNX(W_,6) = DYNX(W_,1)-DYNX(W_,7);
 /* Linear system of equations to solve. */
DYNX(F_,0) = RememberSimple_(DYNX(F_,0), 0);
SolveScalarLinearParametric(DYNX(DP_,3),"dCMachine.inductor.L", DYNX(W_,6),
  "dCMachine.inductor.v", DYNX(F_,0),"der(dCMachine.inductor.i)");
 /* End of Equation Block */ 

DYNX(W_,10) =  -DYNX(DP_,4)*DYNX(X_,0);
 /* Linear system of equations to solve. */
DYNX(F_,2) = RememberSimple_(DYNX(F_,2), 1);
SolveScalarLinearParametric(DYNX(DP_,6),"dCMachine.inertia.J",  -DYNX(W_,10),
  " -dCMachine.emf.tau", DYNX(F_,2),"der(dCMachine.inertia.w)");
 /* End of Equation Block */ 
...
AcceptedSection2
DYNX(W_,9) = DYNX(X_,1)-DYNX(DP_,5);
...
"dCMachine.resistor.n.v", 1, 5, 1, 4)
...
DeclareDerivative("dCMachine.inductor.der(i)", "der(Current flowing from pin p to pin n) [A/s]",\
 0.0, 0.0,0.0,0.0,0,512)
DeclareParameter("dCMachine.inductor.L", "Inductance [H]", 3, 1, 0.0,0.0,0.0,0,560)
...
"dCMachine.inductor.n.v", 1, 5, 7, 0)
...
DeclareParameter("dCMachine.inertia.J", "Moment of inertia [kg.m2]", 6, 1, 0.0,\
1E+100,0.0,0,560)
...
DeclareAlias2("dCMachine.flange.phi", "Absolute rotation angle of flange [rad|deg]",\
 "dCMachine.inertia.phi", 1, 1, 1, 4)
DeclareVariable("dCMachine.flange.tau", "Cut torque in the flange [N.m]", 0.0, \
0.0,0.0,0.0,0,777)
...
"dCMachine.inductor.i", -1, 1, 0, 0)
DeclareParameter("step.height", "Height of step [V]", 7, 1, 0.0,0.0,0.0,0,560)
DeclareAlias2("step.y", "Connector of Real output signal [V]", "dCMachine.p.v", 1,\
 5, 17, 0)
DeclareParameter("step.offset", "Offset of output signal y [V]", 8, 0, 0.0,0.0,\
0.0,0,560)
